  package ar.edu.utn.frba.dds.dondeinvierto.server;
  ...
  import ar.edu.utn.frba.dds.dondeinvierto.jobs.Job;
		...
		Spark.get("/indicadores", (req, res) -> {
			...
			if (req.cookie("idUsuarioActivo").toString().length() > 0) {
				...
				if (req.queryParams("mySelectIndicadores") != "" && req.queryParams("mySelectEmpresas") != ""
					...
					// pasarlos al mÃ³dulo Aplicacion
					// TODO: Llamar a evaluarIndicador acÃ¡ y mostrarlo.
				}
